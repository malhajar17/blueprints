...
flexai training run $TRAINING_NAME -D ci-gpt2-tokenized-wikitext -u $SOURCE --checkpoint ci-gpt2-ckpt5 -b $TRAINING_REVISION -- code/causal-language-modeling/train.py \
    --do_eval \
    --do_train \
...
grep "Continuing training from global step 5" logs.txt
